...
const storeMemorySchema = z.object({
  ...
  category: z
    .string()
    .optional()
    .describe("Memory category (e.g., code, documentation, conversation)"),
  ...
  ttl: z
    ...
    .optional()
    .describe(
      "Time-to-live in milliseconds (optional, for temporary memories)",
    ),
});
...
async function handleStoreMemory(
  ...
  console.log(
    `Memory-tool handleStoreMemory with input ${input} and context: ${JSON.stringify(context)}`,
  );
  ...
  if (resourceManager?.memoryService) {
    try {
      ...
      const memoryService =
        resourceManager.memoryService as UnifiedMemoryService;
...
export const storeMemoryTool: MCPTool = {
  name: "memory/store",
  description:
    "Store a memory entry with specified type, category, and metadata",
  inputSchema: {
    type: "object",
    properties: {
      content: {
      ...
      category: {
        type: "string",
        description:
          "Memory category (e.g., code, documentation, conversation)",
      },
      tags: {
        type: "array",
        items: { type: "string" },
...
export const searchMemoryTool: MCPTool = {
  name: "memory/search",
  description:
    "Search memories using semantic and keyword search with filtering",
  inputSchema: {
    type: "object",
    properties: {
      query: {
